...
export const command = (
  ...
  async execute(): Promise<CliExitCode> {
    ...
    if (workspace.moveToCommon) {
      await workspace.moveToCommon(selectors, gp)
...
const initBuilder = createCommandBuilder({
  options: {
    command: 'moveToCommon [selector]',
    description: 'Moves a Salto element to common',
    positional: {
      selector: {
        type: 'string',
        description: 'Elem ID of the element to move to common (or regex if you feel like it)',
