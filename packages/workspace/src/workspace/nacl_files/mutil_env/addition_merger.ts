...
import {
  ...
  isObjectType, isInstanceElement, PrimitiveType, isField, FieldDefinition, Field, DetailedChange,
...
const createObjectTypeFromNestedAdditions = (
  ...
  new ObjectType(additions.reduce((prev, addition) => {
    ...
    fields: _.mapValues(
      commonObjectType.fields,
      f => ({ type: f.type, annotations: f.annotations })
    ) as Record<string, FieldDefinition>,
    annotationTypes: commonObjectType.annotationTypes,
    annotations: commonObjectType.annotations,
...
export const mergeAdditionElement = (
  additions: DetailedAddition[],
...
export const mergeAdditionWithTarget = (
  nestedAdditions: DetailedAddition[],
  commonElement: Element,
): DetailedChange => {
  const refAddition = nestedAdditions[0]
  const wrapperElement = mergeAdditionElement(nestedAdditions, commonElement)
  return {
    action: 'modify',
